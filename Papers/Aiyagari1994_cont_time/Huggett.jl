...
const β=0.9
const σ=1.5
const u(c)= σ==1 ? log.(c) : (c^(1-σ) -1)/(1-σ);
...
a̲   = 1e-10;
Na  = 200;
a_grid   = Array(range(a̲,a̅, length=Na));
...
while conv1 > tol
   ...
   global v1_i=Int[];
   global v2_i=Int[];
   for (i,a_t) in enumerate(a_grid)
       for (j,a_t_1) in enumerate(a_grid)
    #    c1[j]=a_t + e_h - q*a_t_1
    #    c2[j]=a_t + e_l - q*a_t_1

       C[i,:,j]    = a_t + e_states - q*a_t_1


       C[i,1,j] > 0 ?
       V[i,1,j] = u(C[i,1,j]) + β*(π[1,1] *V0[j,1] + π[1,2] *V0[j,1]) :
       V[i,1,j] = -999_999

       C[i,2,j] > 0 ?
       V[i,2,j] = u(C[i,1,j]) + β*(π[2,1] *V0[j,2] + π[2,2] *V0[j,2]) :
       V[i,2,j] = -999_999
       end
      #optimal vf
      v1[i],ind_h=findmax(V,dims=(1))
