	FUNCTION brent(ax,bx,cx,func,tol,xmin,h,is,nb,nss,vv,BB)
	USE nrtype
	IMPLICIT NONE
		REAL(SP), INTENT(IN) :: ax,bx,cx,tol
	REAL(SP), INTENT(OUT) :: xmin
	integer, intent(in) :: h,is , nb,nss
	real(dp), intent(in) :: vv(nb,nss),bB(nb)
	REAL(SP) :: brent
	INTERFACE
		FUNCTION func(x,h,is,nb,nss,vv,BB)
		USE nrtype
		use common_par
		IMPLICIT NONE
			integer, intent(in) :: h,is,nb,nss
