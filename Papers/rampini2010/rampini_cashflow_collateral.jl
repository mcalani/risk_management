# ---------------------------------------------------------------------------- #
#                     Cash Flow-based collateral constraint                    #
# ---------------------------------------------------------------------------- #
...
# A1(s)           = s./10
# ν=2.1
# A1(s;ν= ν)      = s.^ν./10 .-0.08 .- mean(S_grid.^ν./10) .+ 0.3 #volatilidad: mirar ν=0.2 vs ν=1.2
ν=1
...
#Limits where the assumption 1 is valid: (R2>R) : not used
# k_0_L = ((R*q)/(α*θ*(Π'A1(S_grid))))^(1/(α-1))
# k_1_L = (R*q/(α*θ*A2))^(1/(α-1))
# k_1_H = (   (R*q-q)/(α*(A2*(1-θ)+θ*A2))   )^(1/(α-1)) 
# w_0_L = (q - inv(R)*θ*(Π'*A1(S_grid))*∂f∂k(k_0_L))*k_0_L ##=0
# w_1_L = (q - inv(R)*θ*A2*∂f∂k(k_1_L))*k_1_L ##=0
...
function Rampini_cashflow_collateral(;Nw=Nw,w0_grid=w0_grid,Ns=Ns,S_grid=S_grid,Π=Π,π=π,q=q,β=β,θ=θ,α=α,R=R,A2=A2,tol=tol)
    ...
    for (j,w) in enumerate(w0_grid)
        ...
        if sum((λ1(w0,S_grid)) .< tol) == 0 # {=1 if λ_1 ≤ 0 ∀ s ∈ S; =0 otherwise}
            ...
            W2[:,j] = A2.*f.(K1[:,j]).*(1-θ) .+ q.*K1[:,j] .+ H1[:,j]
    ...
    # ---------------------------------------------------------------------------- #
    @assert 0 < sum(L1[:,J] .≤ 0) < Ns "Constraint is binding in all states. Increase w0_grid!"  # {=1 if ∃ s' : λ_1(s') = 0, s' ∈ S ; =0 otherwise}
    ...
    for (j,w) in enumerate(w0_grid[J2:end])
        ...
        # par_init= θ*A1(S_grid)*f(w0/γ0(K0[j-1])) *inv(R).* [0.8 , 0.7, 0.6, 0.5, 0.4 ]
        par_init= B1[:,j-1]
...
# cd(dirname(@__FILE__))
# pl_h1=plot(w0_grid,H1', palette=col,legend=:none,title=L"h_{1}(s)", framestyle = :box)
# savefig(pl_h1,"./Figures/vol_low_CC.pdf")
# savefig(pl_h1,"./Figures/vol_high_CC.pdf")
...
vspan!([w0_grid[ind_w0_NH],w0_grid[ind_w0_PH]], linecolor = :lightgreen,
         fillcolor = :lightgreen,fillalpha = alpha, label="Partially hedged")
vspan!([w0_grid[ind_w0_PH],w0_max], linecolor = :green, fillcolor = :green,
        fillalpha = alpha, label="Fully hedged")

# vline!([w0_grid[ind_w0_NH]],linewidth=2,color=:red, label="No hedging") 
# vline!([w0_grid[ind_w0_PH]],linewidth=2,color=:gold, label="Partially hedged") 

p1=plot(p,size=(600,400))
cd(dirname(@__FILE__))
savefig(p,"./Figures/dist_cashflow.pdf")
...
# #CHECKING ASSUMPTION 1: R2 > R, ∀s ∈ S

# RET1(k) = (A1(S_grid)*∂f∂k(k)*(1-θ) .+ q)/ (q - inv(R)*θ*(Π'*A1(S_grid))*∂f∂k(k))
# RET2(k) = (A2*(1-θ)*∂f∂k(k) .+ q)/ (q - inv(R)*θ*A2*∂f∂k(k))

# # RET1_orig(k) = (A1(S_grid)*∂f∂k(k) .+ q*(1-θ))/ γ
# RET2_orig(k) = (A2*∂f∂k(k) .+ q*(1-θ))/ (q .- inv(R).*(q).*θ)

# k_grid = 0.8:0.001:5
# Rs = repeat([R],length(k_grid),1)

# #R2 comparision
# RET2s = RET2.(k_grid)
# RET2s_orig = RET2_orig.(k_grid)
# plot(k_grid,[RET2s , RET2s_orig, Rs], title="R2", label = ["R2 (cash flow-based)" "R2 (capital-based)" "R"])
...
#R1 comparision
# RET1s = mapreduce(permutedims,vcat,RET1.(k_grid))
# RET1s_orig = mapreduce(permutedims,vcat,RET1_orig.(k_grid))

# plot(k_grid,[RET1s , Rs], title="R1", legend = :none)
# plot(k_grid,[RET1s_orig , Rs], title="R1", legend = :none)

# #CUAL ES LA REGION FACTIBLE DE w0 ???

# # Limite inferior de R1


# # Limites de R2
# lim_inf_k0 = ((R*q)/(α*θ*(Π'A1(S_grid))))^(1/(α-1))
# RET1(lim_inf_k0-0.01)



# #lim inf
# lim_inf_k1 = (R*q/(α*θ*A2))^(1/(α-1))
# RET2(lim_inf_k1)


# #lim sup
# lim_sum_k1 = (   (R*q-q)/(α*(A2*(1-θ)+θ*A2))   )^(1/(α-1)) 
# RET2(lim_sum_k1) - R
